...
**M6-A2 CLOSED — Qwen3.8-27B projection compatibility audit**

MIInfer now has a completed M6-A0 audit, a real M6-A1 fixture, and an M6-A2
read-only projection/kernel compatibility map for the selected
`Qwen3.8-27B-Q4_K_M.gguf` artifact. The fixture is generated by the pinned
local llama.cpp CPU reference and contains selected F32 checkpoints, logits,
tokenizer IDs/text, and opaque recurrent/KV state snapshots at positions 0,
1, 2, 4, 8, 16, 32, and 64. The next task is one externally validated
