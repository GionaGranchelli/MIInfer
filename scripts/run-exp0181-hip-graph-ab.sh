...
BASE_FLAGS="MIINFER_Q4K_NATIVE_DOWN=1 MIINFER_Q4K_NATIVE_GATE_UP=1 MIINFER_Q4K_NATIVE_Q=1 MIINFER_Q4K_NATIVE_ATTN_GATE=1 MIINFER_Q4K_NATIVE_ATTN_OUT=1 MIINFER_Q4K_NATIVE_K=1 MIINFER_Q5K_NATIVE_SSM_OUT=1 MIINFER_KQUANT_NATIVE_QKV=1 MIINFER_KQUANT_NATIVE_V=1 MIINFER_Q6K_NATIVE_DOWN=1"

run_suite() {
    local TOKENS=$1
    local SUITE_NAME="tg${TOKENS}"
    local TIMESTAMP=$(date -u +%Y%m%dT%H%M%SZ)
    local OUTDIR="bench/results/exp0181-hip-graph-${SUITE_NAME}/${TIMESTAMP}"
    mkdir -p "$OUTDIR"

    echo "=== Running EXP-0181 Static HIP Graph A/B benchmark for TG${TOKENS} ==="
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (EXP-0179, HIP_GRAPH=0)..."
        env $BASE_FLAGS MIINFER_HIP_GRAPH=0 \
        ...
        echo "[Pair $i/5] Running Candidate (EXP-0181, HIP_GRAPH=1)..."
        env $BASE_FLAGS MIINFER_HIP_GRAPH=1 \
...
print(f'Control  (HIP_GRAPH=0): {med_ctrl_tok:.4f} tok/s | {med_ctrl_lat:.3f} ms/token (median)')
print(f'Candidate(HIP_GRAPH=1): {med_cand_tok:.4f} tok/s | {med_cand_lat:.3f} ms/token (median)')
