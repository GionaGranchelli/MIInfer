...
if [[ ! -f "$model_path" ]]; then
    printf 'M5-C6c benchmark: model does not exist: %s\n' "$model_path" >&2
    exit 2
fi
...
if [[ ! -x "$benchmark" ]]; then
    printf 'M5-C6c benchmark executable is missing: %s\n' "$benchmark" >&2
    printf 'build it with: cmake --build --preset mi50-release --target miinfer-qwen3-attention-ab-bench\n' >&2
    exit 2
fi

exec "$repo_root/scripts/run-bench.sh" "$benchmark" "$model_path" --mode kv-cache
