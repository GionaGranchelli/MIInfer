...
run_suite() {
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (EXP-0196 Baseline: MIINFER_SWIGLU_PAIRED=0)..."
        env $BASE_FLAGS MIINFER_SWIGLU_PAIRED=0 \
        ...
        echo "[Pair $i/5] Running Candidate (EXP-0197 Paired SwiGLU: MIINFER_SWIGLU_PAIRED=1)..."
        env $BASE_FLAGS MIINFER_SWIGLU_PAIRED=1 \
    ...
    echo "Control runs (Separate Gate/Up Buffers):"
    for i in 1 2 3 4 5; do
        awk -F' ' '{print "  Run " '"$i"' ": " $0}' "${OUTDIR}/control-${i}.log" | grep "benchmark_tokens="
    done
    echo "Candidate runs (EXP-0197 Paired SwiGLU Tiles):"
    for i in 1 2 3 4 5; do
...
print(f'Control  (Separate Gate/Up):   {med_ctrl_tok:.4f} tok/s | {med_ctrl_lat:.3f} ms/token (median)')
print(f'Candidate(Paired SwiGLU Tile): {med_cand_tok:.4f} tok/s | {med_cand_lat:.3f} ms/token (median)')
