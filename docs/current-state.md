...
**M6-A1 CLOSED — Qwen3.8-27B external reference fixture**

MIInfer now has a completed M6-A0 audit and a real M6-A1 fixture for the
selected `Qwen3.8-27B-Q4_K_M.gguf` artifact. The fixture is generated by the
pinned local llama.cpp CPU reference and contains selected F32 checkpoints,
logits, tokenizer IDs/text, and opaque recurrent/KV state snapshots at
positions 0, 1, 2, 4, 8, 16, 32, and 64. The next task is the read-only
projection/kernel compatibility audit before implementing the qwen35 hybrid
path.
...
> M6-A2: build a read-only projection/kernel compatibility table for the
> selected Qwen3.8-27B GGUF and current MIInfer primitives. No DeltaNet or
> production-kernel implementation is selected until compatibility is mapped.
