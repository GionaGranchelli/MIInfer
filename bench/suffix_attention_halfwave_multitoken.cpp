...
__global__ void qwen35_splitk_suffix_attn_stage2_kernel(
    ...
    const std::uint32_t token_head = blockIdx.x; // [0, token_count * query_heads)
    const std::uint32_t lane = threadIdx.x;      // [0, 64)

    if (token_head >= token_count * query_heads || lane >= 64) return;
    ...
    if (lane == 0) {
        ...
        for (std::uint32_t s = 0; s < num_splits; ++s) {
            const std::size_t split_idx =
                static_cast<std::size_t>(s) * (token_count * query_heads) + token_head;
    ...
    for (std::uint32_t s = 0; s < num_splits; ++s) {
        ...
        if (l > 0.0f && std::isfinite(m)) {
            ...
            acc.x += partial.x * alpha;
            acc.y += partial.y * alpha;
            acc.z += partial.z * alpha;
            acc.w += partial.w * alpha;
        }
    }
    ...
    const std::size_t out_base = static_cast<std::size_t>(token_head) * head_dim + dim_offset;
    if (gate != nullptr) {
...
// -----------------------------------------------------------------------------
// Stage 1: Ultra-Fast Half-Wave Dual-Head (Zero Shuffle Softmax, Pre-Scaled Q)
// -----------------------------------------------------------------------------
template <int UNROLL = 4>
__global__ void qwen35_splitk_suffix_attn_stage1_ultrafast_kernel(
    const float* __restrict__ q,
    const __half* __restrict__ key_cache,
    const __half* __restrict__ value_cache,
    ...
    float4 q_vec0 = *reinterpret_cast<const float4*>(q + q_base + 0);
    float4 q_vec1 = *reinterpret_cast<const float4*>(q + q_base + 4);

    // Pre-scale Q to eliminate multiplication inside the inner loop
    q_vec0.x *= scale; q_vec0.y *= scale; q_vec0.z *= scale; q_vec0.w *= scale;
    q_vec1.x *= scale; q_vec1.y *= scale; q_vec1.z *= scale; q_vec1.w *= scale;
    ...
    for (; pos + UNROLL <= k_end; pos += UNROLL) {
        ...
        // Half-wave reductions across 32 lanes. All 32 lanes obtain the identical score.
        #pragma unroll
        for (int u = 0; u < UNROLL; ++u) {
            float d = dots[u];
            #pragma unroll
        ...
        // Zero-shuffle in-register online softmax across all lanes
        #pragma unroll
        for (int u = 0; u < UNROLL; ++u) {
            const float score = dots[u];
            float alpha = 1.0f;
            float weight = 0.0f;
            if (score > running_max) {
                alpha = expf(running_max - score);
...
int main() {
    ...
    std::cout << "===================================================================\n"
              << "  MIInfer V2-0016: Ultra-Fast HalfWave Suffix Attention Bakeoff\n"
    ...
    std::vector<TestConfig> configs = {
        {"UltraFast HalfWave Unroll=4 Splits=32", 4, 32},
        {"UltraFast HalfWave Unroll=4 Splits=64", 4, 64},
        {"UltraFast HalfWave Unroll=4 Splits=96", 4, 96},
        {"UltraFast HalfWave Unroll=4 Splits=128", 4, 128},
        {"UltraFast HalfWave Unroll=8 Splits=32", 8, 32},
        {"UltraFast HalfWave Unroll=8 Splits=64", 8, 64},
        {"UltraFast HalfWave Unroll=8 Splits=96", 8, 96},
        {"UltraFast HalfWave Unroll=8 Splits=128", 8, 128},
        {"UltraFast HalfWave Unroll=8 Splits=192", 8, 192},
        {"UltraFast HalfWave Unroll=8 Splits=256", 8, 256},
    };
    ...
    for (const auto& cfg : configs) {
        auto run_candidate = [&]() {
            ...
            if (cfg.unroll == 4) {
                hipLaunchKernelGGL(
                    (qwen35_splitk_suffix_attn_stage1_ultrafast_kernel<4>),
            ...
            } else if (cfg.unroll == 8) {
                hipLaunchKernelGGL(
                    (qwen35_splitk_suffix_attn_stage1_ultrafast_kernel<8>),
                    grid, block, 0, nullptr,
                    d_q.ptr, d_k.ptr, d_v.ptr,
                    d_p_max.ptr, d_p_sum.ptr, d_p_acc.ptr,
                    static_cast<std::uint32_t>(S), static_cast<std::uint32_t>(P),
