...
run_suite() {
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (EXP-0190 baseline, MIINFER_FUSED_NORM_Q8=0)..."
        env $BASE_FLAGS MIINFER_FUSED_NORM_Q8=0 \
        ...
        echo "[Pair $i/5] Running Candidate (EXP-0191 Fused Norm Q8, MIINFER_FUSED_NORM_Q8=1)..."
        env $BASE_FLAGS MIINFER_FUSED_NORM_Q8=1 \
    ...
    echo "Control runs (EXP-0190 baseline, MIINFER_FUSED_NORM_Q8=0):"
    for i in 1 2 3 4 5; do
        awk -F' ' '{print "  Run " '"$i"' ": " $0}' "${OUTDIR}/control-${i}.log" | grep "benchmark_tokens="
    done
    echo "Candidate runs (EXP-0191 Fused Norm Q8, MIINFER_FUSED_NORM_Q8=1):"
    for i in 1 2 3 4 5; do
...
print(f'Candidate(Fused Norm Q8):    {med_cand_tok:.4f} tok/s | {med_cand_lat:.3f} ms/token (median)')
