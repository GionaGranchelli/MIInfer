...
run_suite() {
    ...
    local OUTDIR="bench/results/exp0177-q-${SUITE_NAME}/${TIMESTAMP}"
    mkdir -p "$OUTDIR"

    echo "=== Running EXP-0177 Candidate 2 A/B benchmark for TG${TOKENS} ==="
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (Down + Gate/Up, Q=0)..."
        MIINFER_Q4K_NATIVE_DOWN=1 MIINFER_Q4K_NATIVE_GATE_UP=1 MIINFER_Q4K_NATIVE_Q=0 \
        ...
        echo "[Pair $i/5] Running Candidate (Down + Gate/Up + Q=1)..."
