...
run_suite() {
    ...
    local OUTDIR="bench/results/exp0186-fused-rope-norm-${SUITE_NAME}/${TIMESTAMP}"
    mkdir -p "$OUTDIR"

    echo "=== Running EXP-0186 Fused RoPE + Head Norm A/B benchmark for TG${TOKENS} ==="
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (EXP-0185, FUSED_ROPE_NORM=0)..."
        env $BASE_FLAGS MIINFER_FUSED_ROPE_NORM=0 \
        ...
        echo "[Pair $i/5] Running Candidate (EXP-0186, FUSED_ROPE_NORM=1)..."
        env $BASE_FLAGS MIINFER_FUSED_ROPE_NORM=1 \
    ...
    echo "Control runs (FUSED_ROPE_NORM=0):"
    for i in 1 2 3 4 5; do
        awk -F' ' '{print "  Run " '"$i"' ": " $0}' "${OUTDIR}/control-${i}.log" | grep "benchmark_tokens="
    done
    echo "Candidate runs (FUSED_ROPE_NORM=1):"
    for i in 1 2 3 4 5; do
...
print(f'Control  (ROPE_NORM=0): {med_ctrl_tok:.4f} tok/s | {med_ctrl_lat:.3f} ms/token (median)')
print(f'Candidate(ROPE_NORM=1): {med_cand_tok:.4f} tok/s | {med_cand_lat:.3f} ms/token (median)')
