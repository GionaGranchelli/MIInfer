...
M27 prefix reuse now keeps up to eight sparse exact B512 checkpoints under a
3 GiB GPU-state budget and chooses the longest prefix with a radix trie. The
selected restore point is kept recent while a divergent branch is captured.
The current 16K append/replay matrix, branch-return, and invalidation checks
pass. Real Pi 0.85.1 read-tool cycles exercised ~3.6K, ~7.7K, and ~15K
conversation sizes. The ~3.7K tool-result continuation reused 3584 tokens and
processed 134; the ~7.7K continuation reused 7168 and processed 513; the ~15K
continuation reused 14848 and processed 258. A temporary Pi code-edit cycle
also reused its exact 4096-token prefix through read/edit/read tool calls; the
edited helper passed assertions. The feature remains opt-in and timing is not
qualified. Process restart and cross-session behavior remain open. M26 decode
remains paused, not abandoned. See EXP-0350 and EXP-0357.
...
`45.56 tok/s` (P128 `45.28 tok/s`) versus the fresh matched layer-major
...
a shape-specific word-reuse kernel. EXP-0221 removes prepared-workspace device
copies from the layer-major hot path and measures a further 1.2% P513 gain, but
the 100 tok/s gate remains open; the remaining gap needs a different projection
...
nibbles and decoded scale/minimum metadata. The candidate matches the control
through 64-token generation, passes the external observable contract, and
improves fresh TG64/TG128 medians by `0.47%/0.59%`, at a cost of about
`3.08 GB` persistent VRAM. Set `MIINFER_Q4K_EXPANDED_DOWN=0` to use the packed
control path. See
...
whole-token throughput did not improve. The post-B50 profile records
`73.8636 ms/token` total GPU event and `70.5806 ms/token` layer work at
position 63, with `2.4872 ms` final LM head and zero decode allocations. See
