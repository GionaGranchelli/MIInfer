...
AccuracyResult compute_accuracy(const std::vector<float>& cand, const std::vector<float>& ref) {
    ...
    std::cout << " [DEBUG] norm_cand=" << norm_cand << " norm_ref=" << norm_ref << " dot=" << dot << " ref[0]=" << ref[0] << " cand[0]=" << cand[0] << "\n";
    return res;
}
...
// -----------------------------------------------------------------------------
// Highly Optimized Dual-Issue Half-Wave Kernel (Pointer-Incremented, Full MLP)
// -----------------------------------------------------------------------------
template <int UNROLL = 4>
__global__ void qwen35_splitk_suffix_attn_optimized_halfwave_kernel(
    const float* __restrict__ q,
    const __half* __restrict__ key_cache,
    const __half* __restrict__ value_cache,
    ...
    // Dual-Issue Pipelined Loop: issues 4 Key loads and 4 Value loads simultaneously
...
int main() {
    ...
    std::cout << "===================================================================\n"
              << "  MIInfer V2-0017: Pipelined Memory Stream Bakeoff\n"
    ...
    for (std::size_t i = 0; i < h_q.size(); ++i) {
        h_q[i] = static_cast<float>((i % 100) - 50) * 0.001f;
        h_gate[i] = static_cast<float>((i % 70) - 35) * 0.001f;
    }
    ...
    for (std::size_t i = 0; i < h_kv.size(); ++i) {
        h_kv[i] = __float2half(static_cast<float>((i % 80) - 40) * 0.0005f);
    }
    ...
    miinfer::launch_qwen35_tiled_online_attention_batch_f16(
        ...
        kQueryHeads, kKvHeads, kHeadDim, kScale, nullptr);
    ...
    std::vector<TestConfig> configs = {
        {"Dual-Issue Pipelined HalfWave (Splits=32)", 32},
        {"Dual-Issue Pipelined HalfWave (Splits=64)", 64},
        {"Dual-Issue Pipelined HalfWave (Splits=96)", 96},
        {"Dual-Issue Pipelined HalfWave (Splits=128)", 128},
        {"Dual-Issue Pipelined HalfWave (Splits=192)", 192},
        {"Dual-Issue Pipelined HalfWave (Splits=256)", 256},
    };
    ...
    for (const auto& cfg : configs) {
        auto run_candidate = [&]() {
            ...
            hipLaunchKernelGGL(
                (qwen35_splitk_suffix_attn_optimized_halfwave_kernel<4>),
