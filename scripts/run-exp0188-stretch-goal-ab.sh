...
run_suite() {
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (EXP-0187 baseline, INTERLAYER_NORM=0, NATIVE_LM_HEAD=0)..."
        env $BASE_FLAGS MIINFER_FUSED_INTERLAYER_NORM=0 MIINFER_Q6K_NATIVE_LM_HEAD=0 \
        ...
        echo "[Pair $i/5] Running Candidate (EXP-0188, INTERLAYER_NORM=1, NATIVE_LM_HEAD=1)..."
        env $BASE_FLAGS MIINFER_FUSED_INTERLAYER_NORM=1 MIINFER_Q6K_NATIVE_LM_HEAD=1 \
    ...
    echo "Control runs (EXP-0187 baseline):"
    for i in 1 2 3 4 5; do
        awk -F' ' '{print "  Run " '"$i"' ": " $0}' "${OUTDIR}/control-${i}.log" | grep "benchmark_tokens="
    done
    echo "Candidate runs (EXP-0188 stretch candidate):"
    for i in 1 2 3 4 5; do
...
print(f'Control  (EXP-0187): {med_ctrl_tok:.4f} tok/s | {med_ctrl_lat:.3f} ms/token (median)')
print(f'Candidate(EXP-0188): {med_cand_tok:.4f} tok/s | {med_cand_lat:.3f} ms/token (median)')
print(f'Throughput Delta:    {pct_tps:+.2f}%')
print(f'Latency Saved:       {lat_saved:+.3f} ms/token ({pct_lat:+.2f}%)')
if med_cand_tok >= 28.50:
    print('STRETCH GATE STATUS: >>> PASSED! (TG >= 28.50 tok/s) <<<')
else:
    print(f'STRETCH GATE STATUS: Distance to 28.50 tok/s = {28.50 - med_cand_tok:.2f} tok/s ({med_cand_lat - 35.088:.3f} ms)')
print('---------------------------------------------------------')
"
