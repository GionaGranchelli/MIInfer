...
run_suite() {
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (EXP-0184, TILED_ATTENTION=0)..."
        env $BASE_FLAGS MIINFER_TILED_ONLINE_ATTENTION=0 \
        ...
        echo "[Pair $i/5] Running Candidate (EXP-0185, TILED_ATTENTION=1)..."
        env $BASE_FLAGS MIINFER_TILED_ONLINE_ATTENTION=1 \
    ...
    echo "Control runs (TILED_ATTENTION=0):"
    for i in 1 2 3 4 5; do
        awk -F' ' '{print "  Run " '"$i"' ": " $0}' "${OUTDIR}/control-${i}.log" | grep "benchmark_tokens="
    done
    echo "Candidate runs (TILED_ATTENTION=1):"
    for i in 1 2 3 4 5; do
...
print(f'Control  (ATTENTION=0): {med_ctrl_tok:.4f} tok/s | {med_ctrl_lat:.3f} ms/token (median)')
print(f'Candidate(ATTENTION=1): {med_cand_tok:.4f} tok/s | {med_cand_lat:.3f} ms/token (median)')
