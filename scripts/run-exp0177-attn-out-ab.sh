...
run_suite() {
    ...
    local OUTDIR="bench/results/exp0177-attn-out-${SUITE_NAME}/${TIMESTAMP}"
    mkdir -p "$OUTDIR"

    echo "=== Running EXP-0177 Candidate 4 A/B benchmark for TG${TOKENS} ==="
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (Down+GateUp+Q+AttnGate, ATTN_OUT=0)..."
        MIINFER_Q4K_NATIVE_DOWN=1 MIINFER_Q4K_NATIVE_GATE_UP=1 MIINFER_Q4K_NATIVE_Q=1 MIINFER_Q4K_NATIVE_ATTN_GATE=1 MIINFER_Q4K_NATIVE_ATTN_OUT=0 MIINFER_Q4K_NATIVE_K=0 \
        ...
        echo "[Pair $i/5] Running Candidate (Down+GateUp+Q+AttnGate+AttnOut)..."
