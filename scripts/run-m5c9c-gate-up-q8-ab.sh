...
if [[ ! -f "$model_path" ]]; then
    printf 'M5-C9c benchmark: model does not exist: %s\n' "$model_path" >&2
    exit 2
fi
...
if [[ ! -x "$benchmark" ]]; then
    printf 'M5-C9c benchmark executable is missing: %s\n' "$benchmark" >&2
...
exec "$repo_root/scripts/run-bench.sh" "$benchmark" "$model_path" \
    --mode gate-up-q8-reuse --warmup 8 --generated-tokens 64 --pairs 3
