...
run_suite() {
    ...
    local OUTDIR="bench/results/exp0182-fused-gate-up-${SUITE_NAME}/${TIMESTAMP}"
    mkdir -p "$OUTDIR"

    echo "=== Running EXP-0182 Fused Gate+Up SwiGLU A/B benchmark for TG${TOKENS} ==="
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (EXP-0181, FUSED_GATE_UP_SWIGLU=0)..."
        env $BASE_FLAGS MIINFER_FUSED_GATE_UP_SWIGLU=0 \
        ...
        echo "[Pair $i/5] Running Candidate (EXP-0182, FUSED_GATE_UP_SWIGLU=1)..."
        env $BASE_FLAGS MIINFER_FUSED_GATE_UP_SWIGLU=1 \
    ...
    echo "Control runs (FUSED_GATE_UP_SWIGLU=0):"
    for i in 1 2 3 4 5; do
        awk -F' ' '{print "  Run " '"$i"' ": " $0}' "${OUTDIR}/control-${i}.log" | grep "benchmark_tokens="
    done
    echo "Candidate runs (FUSED_GATE_UP_SWIGLU=1):"
    for i in 1 2 3 4 5; do
...
print(f'Control  (FUSED=0): {med_ctrl_tok:.4f} tok/s | {med_ctrl_lat:.3f} ms/token (median)')
print(f'Candidate(FUSED=1): {med_cand_tok:.4f} tok/s | {med_cand_lat:.3f} ms/token (median)')
