...
run_suite() {
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (M8/M9 separate projections: MIINFER_COMBINED_QKV_GATE=0 MIINFER_COMBINED_ATTN_QK=0)..."
        env $BASE_FLAGS MIINFER_COMBINED_QKV_GATE=0 MIINFER_COMBINED_ATTN_QK=0 \
        ...
        echo "[Pair $i/5] Running Candidate (EXP-0196 Combined projections: MIINFER_COMBINED_QKV_GATE=1 MIINFER_COMBINED_ATTN_QK=1)..."
        env $BASE_FLAGS MIINFER_COMBINED_QKV_GATE=1 MIINFER_COMBINED_ATTN_QK=1 \
    ...
    echo "Control runs (Separate projections):"
    for i in 1 2 3 4 5; do
        awk -F' ' '{print "  Run " '"$i"' ": " $0}' "${OUTDIR}/control-${i}.log" | grep "benchmark_tokens="
    done
    echo "Candidate runs (EXP-0196 Combined projections):"
    for i in 1 2 3 4 5; do
...
print(f'Control  (Separate Projections):  {med_ctrl_tok:.4f} tok/s | {med_ctrl_lat:.3f} ms/token (median)')
print(f'Candidate(Combined Projections):  {med_cand_tok:.4f} tok/s | {med_cand_lat:.3f} ms/token (median)')
