...
run_suite() {
    ...
    local OUTDIR="bench/results/exp0177-gate-up-${SUITE_NAME}/${TIMESTAMP}"
    mkdir -p "$OUTDIR"

    echo "=== Running EXP-0177 Candidate 1 A/B benchmark for TG${TOKENS} ==="
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (Native Down only)..."
        MIINFER_Q4K_NATIVE_DOWN=1 MIINFER_Q4K_NATIVE_GATE_UP=0 \
        ...
        echo "[Pair $i/5] Running Candidate (Native Down + Native Gate/Up)..."
        MIINFER_Q4K_NATIVE_DOWN=1 MIINFER_Q4K_NATIVE_GATE_UP=1 \
