...
run_suite() {
    ...
    local OUTDIR="bench/results/exp0189-wave64-q8-${SUITE_NAME}/${TIMESTAMP}"
    mkdir -p "$OUTDIR"

    echo "=== Running EXP-0189 M8 Lane 1 Wave64-Q8 A/B benchmark for TG${TOKENS} ==="
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (M7 baseline, MIINFER_WAVE64_Q8=0)..."
        env $BASE_FLAGS MIINFER_WAVE64_Q8=0 \
        ...
        echo "[Pair $i/5] Running Candidate (EXP-0189 Wave64 Q8, MIINFER_WAVE64_Q8=1)..."
        env $BASE_FLAGS MIINFER_WAVE64_Q8=1 \
    ...
    echo "Control runs (M7 baseline, MIINFER_WAVE64_Q8=0):"
    for i in 1 2 3 4 5; do
        awk -F' ' '{print "  Run " '"$i"' ": " $0}' "${OUTDIR}/control-${i}.log" | grep "benchmark_tokens="
    done
    echo "Candidate runs (EXP-0189 Wave64 Q8, MIINFER_WAVE64_Q8=1):"
    for i in 1 2 3 4 5; do
...
print(f'Control  (M7 Baseline): {med_ctrl_tok:.4f} tok/s | {med_ctrl_lat:.3f} ms/token (median)')
print(f'Candidate(Wave64 Q8):   {med_cand_tok:.4f} tok/s | {med_cand_lat:.3f} ms/token (median)')
