...
run_suite() {
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (M8 baseline, MIINFER_KQUANT_FAST_ARITH=0)..."
        env $BASE_FLAGS MIINFER_KQUANT_FAST_ARITH=0 \
        ...
        echo "[Pair $i/5] Running Candidate (EXP-0194 Fast Arith, MIINFER_KQUANT_FAST_ARITH=1)..."
        env $BASE_FLAGS MIINFER_KQUANT_FAST_ARITH=1 \
    ...
    echo "Control runs (M8 baseline, MIINFER_KQUANT_FAST_ARITH=0):"
    for i in 1 2 3 4 5; do
        awk -F' ' '{print "  Run " '"$i"' ": " $0}' "${OUTDIR}/control-${i}.log" | grep "benchmark_tokens="
    done
    echo "Candidate runs (EXP-0194 Fast Arith, MIINFER_KQUANT_FAST_ARITH=1):"
    for i in 1 2 3 4 5; do
...
print(f'Control  (M8 Baseline):           {med_ctrl_tok:.4f} tok/s | {med_ctrl_lat:.3f} ms/token (median)')
print(f'Candidate(EXP-0194 Fast Arith):   {med_cand_tok:.4f} tok/s | {med_cand_lat:.3f} ms/token (median)')
