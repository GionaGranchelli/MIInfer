...
run_suite() {
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (EXP-0186, FUSED_ADD_RMS_NORM=0)..."
        env $BASE_FLAGS MIINFER_FUSED_ADD_RMS_NORM=0 \
        ...
        echo "[Pair $i/5] Running Candidate (EXP-0187, FUSED_ADD_RMS_NORM=1)..."
        env $BASE_FLAGS MIINFER_FUSED_ADD_RMS_NORM=1 \
    ...
    echo "Control runs (FUSED_ADD_RMS_NORM=0):"
    for i in 1 2 3 4 5; do
        awk -F' ' '{print "  Run " '"$i"' ": " $0}' "${OUTDIR}/control-${i}.log" | grep "benchmark_tokens="
    done
    echo "Candidate runs (FUSED_ADD_RMS_NORM=1):"
    for i in 1 2 3 4 5; do
...
print(f'Control  (ADD_NORM=0): {med_ctrl_tok:.4f} tok/s | {med_ctrl_lat:.3f} ms/token (median)')
print(f'Candidate(ADD_NORM=1): {med_cand_tok:.4f} tok/s | {med_cand_lat:.3f} ms/token (median)')
