...
run_suite() {
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (EXP-0183, FAST_ARGMAX=0)..."
        env $BASE_FLAGS MIINFER_FAST_ARGMAX=0 \
        ...
        echo "[Pair $i/5] Running Candidate (EXP-0184, FAST_ARGMAX=1)..."
        env $BASE_FLAGS MIINFER_FAST_ARGMAX=1 \
    ...
    echo "Control runs (FAST_ARGMAX=0):"
    for i in 1 2 3 4 5; do
        awk -F' ' '{print "  Run " '"$i"' ": " $0}' "${OUTDIR}/control-${i}.log" | grep "benchmark_tokens="
    done
    echo "Candidate runs (FAST_ARGMAX=1):"
    for i in 1 2 3 4 5; do
...
print(f'Control  (ARGMAX=0): {med_ctrl_tok:.4f} tok/s | {med_ctrl_lat:.3f} ms/token (median)')
print(f'Candidate(ARGMAX=1): {med_cand_tok:.4f} tok/s | {med_cand_lat:.3f} ms/token (median)')
