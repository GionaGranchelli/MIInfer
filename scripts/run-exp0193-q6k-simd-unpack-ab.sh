...
run_suite() {
    ...
    for i in 1 2 3 4 5; do
        echo "[Pair $i/5] Running Control (EXP-0192 baseline, MIINFER_Q6K_SIMD_UNPACK=0)..."
        env $BASE_FLAGS MIINFER_Q6K_SIMD_UNPACK=0 \
        ...
        echo "[Pair $i/5] Running Candidate (EXP-0193 SIMD Q6_K Unpack, MIINFER_Q6K_SIMD_UNPACK=1)..."
        env $BASE_FLAGS MIINFER_Q6K_SIMD_UNPACK=1 \
    ...
    echo "Control runs (EXP-0192 baseline, MIINFER_Q6K_SIMD_UNPACK=0):"
    for i in 1 2 3 4 5; do
        awk -F' ' '{print "  Run " '"$i"' ": " $0}' "${OUTDIR}/control-${i}.log" | grep "benchmark_tokens="
    done
    echo "Candidate runs (EXP-0193 SIMD Q6_K Unpack, MIINFER_Q6K_SIMD_UNPACK=1):"
    for i in 1 2 3 4 5; do
...
print(f'Control  (EXP-0192 Baseline):      {med_ctrl_tok:.4f} tok/s | {med_ctrl_lat:.3f} ms/token (median)')
print(f'Candidate(SIMD Q6_K Unpack):       {med_cand_tok:.4f} tok/s | {med_cand_lat:.3f} ms/token (median)')
